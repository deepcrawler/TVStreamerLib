...
public class SimpleDevicePicker implements ConnectableDeviceListener {
    ...
    protected void loadStringIds() {
        selectDeviceResId = getStringId("TVCast_Lib_picker_select_device");
        simplePairingTitleResId = getStringId("TVCast_Lib_pairing_simple_title_tv");
        simplePairingPromptResId = getStringId("TVCast_Lib_pairing_simple_prompt_tv");
        pinPairingPromptResId = getStringId("TVCast_Lib_pairing_pin_prompt_tv");
        connectionFailedResId = getStringId("TVCast_Lib_connection_failed");
    }
    ...
    protected int getStringId(String key) {
        ...
        // Then try to get from TVCastLib library
        if (id == 0) {
            id = this.activity.getResources().getIdentifier(key, "string", "com.TVCastLib");
        }

        if (id == 0) {
            Log.w("TVCastLib", "missing string resource for \"" + key + "\"");
